...
const GetClusterInputSchema = z
  .object({
    cluster: z
      ...
      .describe(
        "The cluster name or ID. You can use the display name (e.g., 'my-cluster') or the cluster ID.",
      ),
  })
  .strict();
...
const UpdateClusterInputSchema = z
  .object({
    cluster: z
      ...
      .min(1, "Cluster name or ID is required")
      .describe(
        "The cluster name or ID to update. You can use the display name (e.g., 'my-cluster') or the cluster ID.",
      ),
    displayName: z
      .string()
      .max(64, "Display name must not exceed 64 characters")
...
const DeleteClusterInputSchema = z
  .object({
    cluster: z
      ...
      .min(1, "Cluster name or ID is required")
      .describe(
        "The cluster name or ID to delete. You can use the display name (e.g., 'my-cluster') or the cluster ID.",
      ),
  })
  .strict();
...
function formatClusterOutput(cluster: Cluster, title: string): string {
  ...
  if (cluster.spendingLimit) {
    lines.push(`**Spending Limit:** $${cluster.spendingLimit.monthly}/month`);
  }
  ...
  Complete cluster details including endpoints, usage, and configuration.`,
    ...
    async (params: GetClusterInput) => {
      try {
        ...
        const textContent = formatClusterOutput(cluster, "Cluster Details");
...
Args:
  - cluster (string, required): The cluster name or ID to update
  ...
  Updated cluster details.`,
    ...
    async (params: UpdateClusterInput) => {
      try {
        // Resolve cluster name to ID
        const clusterId = await client.resolveClusterId(params.cluster);
        const cluster = await client.updateCluster(clusterId, {
          displayName: params.displayName,
          spendingLimit: params.spendingLimitMonthly
            ? { monthly: params.spendingLimitMonthly }
            : undefined,
...
Args:
  - cluster (string, required): The cluster name or ID to delete
