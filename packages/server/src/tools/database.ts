...
const DbQueryInputSchema = ConnectionOverrideSchema.extend({
    sql: z
        ...
        .describe("The read-only SQL query to execute (SELECT, SHOW, DESCRIBE, EXPLAIN)"),
    database: z
        .string()
        .optional()
        .describe("Database to use for the query"),
}).strict();
...
const DbCreateUserInputSchema = z
    .object({
        ...
        userHost: z
            ...
            .optional()
            .default("%")
            .describe("Host from which the user can connect (default: '%' for any host)"),
    ...
    if (!host || !username || !password) {
        throw new TiDBDatabaseError(
            "Database connection not configured. " +
                "Set TIDB_CLOUD_DB_HOST, TIDB_CLOUD_DB_USER, and TIDB_CLOUD_DB_PASSWORD environment variables, " +
                "or provide host, username, and password parameters.",
        );
    }
