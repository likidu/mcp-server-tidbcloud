 ...
 * protocol handling. Supports both stateless mode (API key) and stateful
 * mode (OAuth sessions).
 */
...
function cleanupExpiredSessions(): void {
    // For now, we don't track creation time in the simple map
    // In production, we'd use Vercel KV with TTL
}

/**
 * Creates the MCP handler for Hono
 *
 * This handler uses the SDK's WebStandardStreamableHTTPServerTransport
 * which properly handles all MCP protocol details including:
